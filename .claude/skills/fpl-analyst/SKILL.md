...
| `dashboard-input.json` | Builder input: squad element ids + C/VC, bank, FTs, chips note, watchlist names, elite panel ids | Every run |
| `dashboard.html` | Rendered FPL-style dashboard (generated by build-dashboard.js — never hand-edit) | Every run |
...
dashboard"). Mechanism: update `coverage/dashboard-input.json` (squad
element ids in lineup order + captain/vice from `get_my_current_team`,
bank, FTs, chips note, watchlist names, elite panel ids), then run
`node .claude/skills/fpl-analyst/assets/build-dashboard.js`. The builder
pulls everything else live from the public FPL API (bootstrap-static,
fixtures, entry picks), computes the projections with the model below, and
replaces ONLY the JSON inside `<script id="fpl-data" type="application/json">`
in the template, writing `coverage/dashboard.html`. The view gameweek is
the first one whose deadline is still in the future, so after a deadline
passes the page rolls to the next GW on the next build; the elite tally
uses the last GW whose deadline has passed. During a run,
